...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters(
			...
			array(
				'menu_order' => __( 'Zadano', 'woocommerce' ),
				'popularity' => __( 'Najpopularnije', 'woocommerce' ),
				'rating'     => __( 'Najbolje ocjenjeno', 'woocommerce' ),
				'date'       => __( 'Najnovije', 'woocommerce' ),
				'price'      => __( 'Cijena rastuća', 'woocommerce' ),
				'price-desc' => __( 'Cijena padajuća', 'woocommerce' ),
			)
		);
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( $product && ( $product->has_attributes() || apply_filters( 'wc_product_enable_dimensions_display', $product->has_weight() || $product->has_dimensions() ) ) ) {
			$tabs['additional_information'] = array(
				'title'    => __( 'Dodatne informacije', 'woocommerce' ),
				'priority' => 20,
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_button_view_cart' ) ) {
	...
	function woocommerce_widget_shopping_cart_button_view_cart() {
		echo '<a href="' . esc_url( wc_get_cart_url() ) . '" class="button wc-forward">' . esc_html__( 'Košarica', 'woocommerce' ) . '</a>';
	}
}
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_proceed_to_checkout' ) ) {
	...
	function woocommerce_widget_shopping_cart_proceed_to_checkout() {
		echo '<a href="' . esc_url( wc_get_checkout_url() ) . '" class="button checkout wc-forward">' . esc_html__( 'Blagajna', 'woocommerce' ) . '</a>';
	}
}
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_subtotal' ) ) {
	...
	function woocommerce_widget_shopping_cart_subtotal() {
		echo '<strong>' . esc_html__( 'Ukupno', 'woocommerce' ) . ':</strong> ' . WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template(
			...
			array(
				...
				'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( 'Naruči', 'woocommerce' ) ),
			)
		);
	}
}
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		$args = wp_parse_args(
			...
			array(
				...
				'id'               => '',
				'class'            => '',
				'show_option_none' => __( 'Odaberite veličinu', 'woocommerce' ),
			)
		);
