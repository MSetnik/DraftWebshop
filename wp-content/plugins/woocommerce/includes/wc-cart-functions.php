...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( 'Proizvod %s je dodan u košaricu', '%s have been added to your cart.', $count, 'woocommerce' ), wc_format_list_of_items( $titles ) );
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message = sprintf( '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s', esc_url( wc_get_cart_url() ), esc_html__( 'Pogledaj košaricu', 'woocommerce' ), esc_html( $added_text ) );
	}
